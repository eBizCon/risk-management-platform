...
public class FinalizeApplicationUpdateConsumerTests
{
    ...
    private readonly FinalizeApplicationUpdateConsumer _consumer;

    public FinalizeApplicationUpdateConsumerTests()
    {
        _consumer = new FinalizeApplicationUpdateConsumer(
    ...
    private static FinalizeApplicationUpdate CreateMessage(Guid correlationId, bool autoSubmit = false)
    {
        return new FinalizeApplicationUpdate(
            correlationId,
            1,
            1,
            5000,
    ...
    [Fact]
    public async Task Consume_ValidRequest_ShouldUpdateDetailsAndPublishCompleted()
    {
        ...
        var contextMock = new Mock<ConsumeContext<FinalizeApplicationUpdate>>();
    ...
    [Fact]
    public async Task Consume_AutoSubmit_ShouldUpdateDetailsAndSubmit()
    {
        var correlationId = Guid.NewGuid();
        var app = CreateProcessingApp();
        var configVersion = CreateConfigVersion();
