...
public class UpdateApplicationHandlerTests
{
    ...
    {
        _handler = new UpdateApplicationHandler(
    ...
    [Fact]
    public async Task HandleAsync_ValidRequest_ShouldSetProcessingAndPublishSagaEvent()
    {
        var app = CreateDraftApp();
        _repositoryMock.Setup(r => r.GetByIdAsync(It.IsAny<AppId>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(app);

        var command = new UpdateApplicationCommand(1, CreateValidDto(), UserEmail);
    ...
    public async Task HandleAsync_WrongUser_ShouldReturnForbidden()
    {
        ...
        var command = new UpdateApplicationCommand(1, CreateValidDto(), "other@test.com");
        var result = await _handler.HandleAsync(command);
