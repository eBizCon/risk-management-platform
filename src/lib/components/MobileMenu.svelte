<script lang="ts">
	import { Home, FileText, ClipboardList, LogOut, X, Settings } from 'lucide-svelte';
		...
		<nav class="flex-1 overflow-y-auto px-4 py-4 space-y-2">
			...
			{#if isAdmin}
				<a
					...
					class="nav-link flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium"
					data-testid="mobile-menu-admin-scoring"
				>
					<Settings class="w-4 h-4" />
					<span>Scoring-Konfiguration</span>
				</a>
			{/if}
		</nav>
		...
		<div class="border-t border-border px-4 py-4 space-y-2">
			{#if user}
				<div class="text-sm">
					...
					<div class="text-secondary">{isAdmin ? 'Administrator' : isApplicant ? 'Antragsteller' : 'Antragsbearbeiter'}</div>
				</div>
				<a
					href="/logout"
