<script lang="ts">
	...
	const errorTestId = $derived(
		status === 401 ? 'auth-error-unauthorized' : status === 403 ? 'auth-error-forbidden' : undefined
	);
</script>
...
<main class="min-h-screen flex items-center justify-center px-4">
	<div class="max-w-lg w-full bg-white shadow-md border border-default rounded-lg p-8 text-center">
		...
		<h1 class="text-2xl font-bold text-primary mb-3" data-testid={errorTestId}>
			{title}
		</h1>
		<p class="text-base text-secondary mb-6">{description}</p>

		<div class="flex flex-col sm:flex-row gap-3 justify-center items-center">
			<a href="/login" class="btn-primary px-5 py-2" data-testid="auth-error-login-link"
				>Zum Login</a
			>
