...
test.describe('Home Page - Applicant View', () => {
	...
	test('should display application dashboard for applicant', async ({ authenticatedPage }) => {
		await authenticatedPage.goto('/');
...
test.describe('Home Page - Processor View', () => {
	...
	test('should display application dashboard for processor', async ({ authenticatedPage }) => {
		const apiResponse = await authenticatedPage.request.get('/api/applications/dashboard-stats');
		baseExpect(apiResponse.ok(), `Dashboard API returned ${apiResponse.status()}: ${await apiResponse.text()}`).toBeTruthy();
...
base.describe('Home Page - Unauthenticated User', () => {
	...
	base('should not display application dashboard for unauthenticated user', async ({ page }) => {
		await clearTestSessions(page);
		await page.goto('/');
