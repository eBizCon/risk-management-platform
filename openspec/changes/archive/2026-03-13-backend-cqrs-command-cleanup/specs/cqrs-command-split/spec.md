...
The system SHALL provide a dedicated `ApproveApplicationCommand` with its own handler that calls `Application.Approve()` on the aggregate. The handler SHALL accept an optional comment.
...
The system SHALL provide a dedicated `RejectApplicationCommand` with its own handler that calls `Application.Reject()` on the aggregate. The handler SHALL require a comment.
...
The system SHALL provide a dedicated `CreateAndSubmitApplicationCommand` with its own handler that creates an application and immediately submits it in a single transaction. The handler SHALL be triggered by a `submit=true` query parameter.
...
The system SHALL provide a dedicated `UpdateAndSubmitApplicationCommand` with its own handler that updates an application and immediately submits it in a single transaction. The handler SHALL be triggered by a `submit=true` query parameter.
...
The `Application` aggregate SHALL expose a `Delete()` method that guards against deletion of non-draft applications and raises an `ApplicationDeletedEvent`. The previous `MarkAsDeleted()` method SHALL be removed.
